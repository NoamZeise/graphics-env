...
namespace Resource
{
  ...
  std::vector<QuadDraw> GLFontLoader::DrawString(Font drawfont,
    ...
    for (std::string::const_iterator c = text.begin(); c != text.end(); c++) {
      ...
      if (cTex == nullptr)
        continue;
      else if (cTex->texture.ID != 0) // if character is added but no
                                      // texture loaded for it (eg space)
      {
  ...
  GLFontLoader::LoadedFont::LoadedFont(std::string file,
  ...
  bool GLFontLoader::LoadedFont::loadCharacter(GLTextureLoader *textureLoader,
                                               FT_Face face, char c) {
    if (FT_Load_Char(face, c, FT_LOAD_RENDER)) {
      std::cout << "error loading " << c << std::endl;
      _chars.insert(std::pair<char, Character *>(
          c, new Character(Resource::Texture(0, glm::vec2(0, 0), "NULL"),
                           glm::vec2(face->glyph->bitmap.width / (double)SIZE,
                                     face->glyph->bitmap.rows / (double)SIZE),
                           glm::vec2(face->glyph->bitmap_left / (double)SIZE,
    ...
    unsigned char *buffer = new unsigned char[face->glyph->bitmap.width *
                                              face->glyph->bitmap.rows * 4];

    int buffIndex = 0;
    char blank = 0x00;
    for (size_t i = 0; i < face->glyph->bitmap.width * face->glyph->bitmap.rows;
         i++) {
